...
After finding a lot of ways, describing data in R, our supervisor suggested to make a cleanup. This makes our report lacking a straight chronology. 
...
We find a dataset containing 6 smaller datasets. Five of them are large dataframes while a sixth one is a list of dataframes. For a better overview we will separate them.
...
We also observe a large variety of tumor diseases beeing recorded by this dataset. Because we want to focuss on breast cancer we will extract the data contributing to this harmful ilness. We identified 28 cell lines, fulfilling this condition.
...
#funktioniert nicht
existingDF <- BCCL_Expression
order.rows <- function(existingDF,orderedDF) { 
    orderedDF <- existingDF[order(rownames(orderedDF)), ]
    orderedDF
}
View(orderedDF)
```
