...
names(SST_cand_ERBB2) = "PIK3CA"

SST_cand_ERBB2 <- SST_cand_ERBB2 %>%rownames_to_column() %>% top_n(15) %>% column_to_rownames() #select 15 genes with highest p-value
SST_cand_MYCBP <- SST_cand_MYCBP %>% rownames_to_column() %>% top_n(15) %>% column_to_rownames()
SST_cand_PARP10 <- SST_cand_PARP10 %>% rownames_to_column() %>% top_n(15) %>% column_to_rownames()
SST_cand_PIK3CA <- SST_cand_PIK3CA %>% rownames_to_column() %>% top_n(15) %>% column_to_rownames()
```
