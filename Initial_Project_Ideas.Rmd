...
BCNumbers <- which(allDepMapData$annotation[4]== "Breast Cancer")
...
BCCL_Expression <- as.data.frame( df1[, c(BCNumbers)])
BCCl_Copynumber <- as.data.frame( df2[, c(BCNumbers)])
BCCL_kd.ceres <- as.data.frame( df3[, c(BCNumbers)])
BCCL_kd.prob <- as.data.frame( df4[, c(BCNumbers)])
dim(BCCL_Expression)
dim(BCCl_Copynumber)
dim(BCCL_kd.ceres)
dim(BCCL_kd.prob)
...
```{r}
summary(BCExpression) # i will start with Expression Matrix but not with much thoughs about it 
