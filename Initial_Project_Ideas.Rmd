...
rownamesold <- rownames(BCCL_kd.ceres) # save old rownames
...
BCCL_kd.ceres <- insertRow(existingDF, newrow, r)
rownames(BCCL_kd.ceres) <- c("Deep_ID", rownamesold) # change rownames back 
BCCL_kd.ceres[1:2, ]
```
...
* **visualize gene expression data from different samples:**
  ...
  ++ R package: GGPlot2 (recommended by David, helpful desriptions & cheat sheets online)
      ...
      installation requires download of latest Rtools version: https://cran.r-project.org/bin/windows/Rtools/ 
...
* **check how relevant driver mutations are for cancer cells using PCA:**
  ...
  ++ maybe also check contribution of second-site targets to PCs, HOWEVER impacht of second-site targets could be smaller as they are non-oncogenic and only act as "co-effectors" regarding the proliferation of tumour cells
