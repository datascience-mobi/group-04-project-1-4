...
To run regression, certain conditions must be fulfilled:
  + Check Normality with a qqPlot
    + Using the aes-function for _aesthetic_ plots, provided by the "ggplot2" package
...
check_assumptions <- function(a){  
  ...
  q <-  round(cor(data_regression[[a]]),2) #create table with correlation values between all SSTs; the values are rounded to two decimal places
  
  return(list(p,q))
   }
lapply(seq_along(data_regression), check_assumptions)
...
### Gibt nur jeweils 1 statt 4 Plots raus, wie kann ich das verbessern?
 ...
 spearmanPlot <- ggscatter(test_set, x = "test_set$Predictor", y = "test_set$Prediction",
                  color = 'blue',   # design the points
                  add = "reg.line", #add the regression line
                  add.params = list(color = "black", fill = "lightgray"),  # design the line
                  conf.int = TRUE, #show confidence interval
                  cor.coef = TRUE, cor.method = "spearman", #output is Spearman correlation coeff.
                  xlab = "Observed CERES score", ylab = "Predicted CERES score") #name x/y axis
