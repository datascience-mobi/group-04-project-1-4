...
We want to predict Ceres scores of our drivermutations ("ERBB2","MYCBP", "PARP10", "PIK3CA") based on data we got from second site targets right ?
...
#Step 2: Check of distribution

To get a first look at the dataset for the Regression model, we create qq-plots to verify that the data is approx. normal distributed.

```{r}
LinRes_ERBB2 <- as.data.frame(LinRes_ERBB2)
LinRes_MYCBP <- as.data.frame(LinRes_MYCBP)
LinRes_PARP10 <- as.data.frame(LinRes_PARP10)
LinRes_PIK3CA <- as.data.frame(LinRes_PIK3CA)
...
df.list.qqplot <- list(LinRes_ERBB2,LinRes_MYCBP, LinRes_PARP10, LinRes_PIK3CA)
lapply(df.list.qqplot, create_qqplot) #tried this, returned Issue as in line 127
...
create_qqplot(linRegData_ERBB2)
create_qqplot(linRegData_MYCBP)
create_qqplot(linRegData_PARP10)
create_qqplot(linRegData_PIK3CA)
#ISSUE: "Error in xy.coords(x, y, xlabel, ylabel, log) : 'x' and 'y' lengths differ", obwohl beide Datensätze gleich sind
View(LinRes_ERBB2)
View(linRegData_ERBB2)
```
