...
create_qqplot <- function(a){  
  p <- ggplot(test[[a]], aes(sample=test[[a]][,ncol(test[[a]])])) + stat_qq()+ stat_qq_line()
   p + labs(title = "QQ-Plot", subtitle = "Check if data is approx. normally distributed")
}
lapply(seq_along(test), create_qqplot)
```
...
lapply(seq_along(input_data), function(a) {
  ...
  return(corVal)
  barplot(corVal, main = deparse(substitute(x)))
})
```
...
lapply(seq_along(input_data), function(a) {
                          set.seed(123) #initialize the random numbers, urgently advised to improve reproducability
                           colnames(data)[length(colnames(data))] <- "Predictor" #The predictors are the SSTs
                          split = sample.split(data[,ncol(data)], SplitRatio = 0.75) #split the dataset into 3/4 Training and 1/4 Testing dataset
                          training_set = subset(x, split == TRUE)
                          test_set = subset(x, split == FALSE)
                          
                          regressor = lm(formula = Predictor ~ ., data=training_set)
                          ...
                          corVal <- cor.test(test_set$Predictor, test_set$Prediction, method = spearman) #to judge model performance, calculate Spearman corr. between predicted and observed values
                          return(corVal) # return p-value plus Corr. coeff. for discussion
                          barplot(corVal, main = deparse(substitute(x))) #create a barplot for correlation visualization and further evaluation
})
...
# Revision: Adding a statistical parameter
  ...
  + Most importantly, it is not possible to assess the strength of the correlation based on the p-value. It rather judges the statistical test than the resulting value.
    + as a standard, set the significance level to 0.05

# Objective
 + We will the compare this values for the different models
    + Best model design is achieved for a rho close to 1.
...
# Discussion
  ...
  + ERBB2-model:
    + Shows the highest rho value of all four models, but ~ 0.26 is still rather weak positive relationship
    + The p-value leads to __acceptance of H0__.
    
  + MYCBP-model:
    + The rho value does not lead to any correlation
    + A p-value of 1 is no good for any kind of statistical work
    
  + PARP10-model:
    + The rho value of ~ -0.54 is a moderate negative relationship.
    + The p-value leads to acceptance of H0.
...
+ The models for MYCBP and PIK3CA did not show any correlation.
