...
km_CERES= kmeans(x = BCCL_Expression_rmv_s, centers = 2, nstart = 10)
...
BCCL_kd.ceres_2 = cbind("Mutation", BCCL_kd.ceres[,2:ncol(BCCL_kd.ceres)]) # insert new column


for(j in nrow(BCCL_kd.ceres_2):1){
  if(rownames(BCCL_kd.ceres_2) %in% rownames(mutImpact_c)){
    j= "No mutation"
   BCCL_kd.ceres_2mut = BCCL_kd.ceres_2[j:1]
  } else {
  }
}
...
for(j in 1:nrow(BCCL_kd.ceres_2)){
  if(rownames(BCCL_kd.ceres_2)[j] %in% rownames(mutImpact_c)){
   BCCL_kd.ceres_2[j,1] = "mutation"
  } else {
    BCCL_kd.ceres_2[j,1] = "no mutation"
  }
}
...
BCCL_kd.ceres_an[c("mutation", tPatients_ID)]#reorder columns (works in console but not for table...strange :-( )
