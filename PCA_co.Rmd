...
km_CERES_an = kmeans(x = BCCL_CERES_an_s, centers = 2, nstart = 10)
table(km_CERES_an$cluster)
fviz_cluster(km_CERES_an, data=BCCL_CERES_an_s, labelsize = 0) # plotting clusters


#LABELING DRIVER MUTATIONS 
# CCND mutation not present in BCCL_kd.ceres_3 matrix but ERBB2, ERBB2IP, MYCBP, PARP10 and PIK3CA found 

drivermut =  matrix(c("ERBB2", "ERBB2IP","MYCBP", "PARP10", "PIK3CA"), nrow=5, ncol=1) # create matrix with driver mutation names

BCCL_kd.ceres_3["Driver mutation"] <- sapply(1:nrow(BCCL_kd.ceres_3), function(j){
     if(rownames(BCCL_kd.ceres_3)[j] %in% drivermut){
         BCCL_kd.ceres_3[j,1] = "driver mutation"
     } else {
         BCCL_kd.ceres_3[j,1] = "no driver mutation"
     }
  })
# labeling driver mutations with "driver mutation in BCCL_kd.ceres_3" 
