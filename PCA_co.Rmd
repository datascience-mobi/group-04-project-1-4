...
# all driver mutations but ERBB2IP in cluster 3 when centers = 4 => hence ERBB2IP eliminated
...
# CCND mutation not present in BCCL_kd.ceres_3 matrix but ERBB2, ERBB2IP, MYCBP, PARP10 and PIK3CA found. ERBB2IP however not in same cluster as other driver mutations - eliminated
...
GOI_MYCBP = cbind(t(GOI[c("MYCBP"),]), "MYCBP")
GOI_PARP10 = cbind(t(GOI[c("PARP10"),]), "PARP10")
GOI_PIK3CA = cbind(t(GOI[c("PIK3CA"),]), "PIK3CA")
GOI_ERBB2 = cbind(t(GOI[c("ERBB2"),]), "ERBB2")
CERES_driver_mut = rbind(GOI_ERBB2, GOI_MYCBP, GOI_PARP10, GOI_PIK3CA)
colnames(CERES_driver_mut) = c("CERES score","Driver mutation")
CERESscore = CERES_driver_mut[,"CERES score"]
Drivermutation = CERES_driver_mut[,"Driver mutation"]
CERES_boxplot = ggplot(CERES_driver_mut, aes(x = Drivermutation, y = CERESscore)) +
        geom_boxplot(aes(fill=Drivermutation), outlier.alpha = 0.7,
                     outlier.colour = "#1F3552", outlier.shape = 20, outlier.size = 0.2) +
