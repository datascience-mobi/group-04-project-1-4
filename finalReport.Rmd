...
The dataset used contains 6 smaller datasets. Five of them are large data frames while the sixth is a list of data frames. For the extraction of this sixth variable we first separated it fom the others and gave it a new strucure, which is easier to handle.
...
The next step was to do some experiments, in order to get a feeling for the distribution of data. 
...
All 67 confirmed SSTs were investigated for already existing drugs and disease association. In general our SSTs revealed a broad status of knowledge, spreading from genes not only associated with any disease to genes with an approved drug available. We classified our SSTs in 3 classes: class 1 containing genes already associated with at least one sort of breast cancer, class 2 consisting of genes with associations to at least one cancer type and class 3, genes with no known connection to cancer. 
The largest group were SSTs of class 1, containing 43 genes followed by class 2 genes with 20 members. The third class was clearly the smallest containing only 4 genes. This confirmed that only a few genes not yet known in the research community were found. All of them except for one, "PDGFRA", have no drug on the market. We decided analyse only those SSTs having associations to more than 40 types of cancer or showing interesting characteristics, for example appearance in more than on driver gene. From this knowledge, we aim to recommend drug design for three to five SSTs.
The last twelve candidates showed different characteristics. A few of them, like APC or FANCA were known as tumorsupressor genes and therefore not further involved. Others showed activity only in neuroektodermal tumors and not ein breast cancer. Therefore we want to present to you our top four cancidates for experimatal drug research:

####TNS3: 
- verified SST in ERBB2 and Parp10. It is suggested to play a role in actin remodeling and associated with 47 tumor diseases.(source are the databases. we should think about a format which is noch that big, we have to cite 3 lines)

####BCL9L:
- known to activate beta catenin, one of the main targets of colorectal cancer and verified in ERBB2. A few other suggested functions in tumorigenesis and 65 associated neoplasms are a reason to investigate therapeutic options. (the databases)

####CREBBP: 
- This transcriptional activator of cAMP responsive genes is a SST of ERBB2. Already known active cell compounds proof the potential, research community is giving to this protein. 176 associated tumor dieases and 463 diseases in general are incentive enough to dare a try. (databases)

####SLC4A7:
-strong associations to breast cancer and already known active cell compounds led our attention to this cotransporter(database). With a lot of knowledge inhibiting transporters in any disease, research community could find a way of inhibiting this protein.
...
lapply(seq_along(data_regression), function(a){  
  ...
  p <- ggplot(data_regression[[a]], aes(sample=data_regression[[a]][,ncol(data_regression[[a]])], col = "cyl")) + stat_qq() +           stat_qq_line() #create qq-Plot with line for comparison to normal distribution +
...
```
In the QQ-Plots, we see that the sample Data of MYCBP and PARP10 is approx. normally distributed; ERBB2 and PIK3CA do not present normal distribution.
As one can see in the Heatmap of the Pearson correlation values, no multicollinearity was detected, all x < 0.80.
...
The aim of this project was to identify genes, so called second site targets, which interact genetically with selected driver mutations to promote cell viability and proliferation in breast cell cancers. Our hope was that these genes could be potential new targets for more specific gene therapies. 
