...
All 67 confirmed SSTs were investigated for already existing drugs and disease association. In general our SSTs revealed a broad spectrum of knowledge, spreading from genes not only associated with any disease to genes with an approved drug available.*verstehe den Satz hier nicht so ganz* We classified our SSTs in 3 classes: class 1 containing genes already associated with at least one sort of breast cancer, class 2 consisting of genes with associations to at least one cancer type and class 3, genes with no known connection to cancer. 
The largest group were SSTs of class 1, containing 43 genes followed by class 2 genes with 20 members. The third class was clearly the smallest containing only 4 genes. This confirmed that only a few genes not yet known in the research community were found. All of them except for one, "PDGFRA", have no drug on the market. We decided analyse only those SSTs having associations to more than 40 types of cancer or showing interesting characteristics. From this knowledge, we aim to recommend drug design for 5 SSTs. 
--> see CoS.Rmd and SST_funtion_and_therapy.csv for further information
...
All four models were tested on these assumptions. The following table provides a quick overview of whether constructed assumptions were met (yes) or not (no). For multicollinarity, all models met the condition r < 0.8.
...
The residual distribution of ERBB2 was closest to normal distribution. The PIK3CA residuals showed still acceptable approximation of normal distribution.MYCBP and PARP10 however failed to show approx. normal residual distribution.
...
The ERBB2-model shows the highest rho value of all four models, but ~ 0.1 is a rather weak positive relationship, but the p-value leads to acceptance of H0. In comparison, the rho value of the MYCBP-model leads to a very weak negative correlation with a p-value of ~ 0.78. This leads to confident acceptance of H0.Further, for the PARP10-model, the rho value of ~ -0.67 is a moderate negative relationship, leading to the acceptance of H0. The p-value is the lowest with ~ 0.11. But even if we would raise the confidence level to 0.10 (which would be a questionable move in statistics), the result would not be signifcant enough in order to accept H1.Finally, the rho value of the PIK3CA-model leads to a very weak negative correlation. The according p-value of ~ 0.78  leads to confident acceptance of H0.

The models for MYCBP and PIK3CA only showed weak, negative correlation, which is not useful for further work with this model. Equally the PARP10 model onl resulted in a moderate negative relationship, which is also not of relevance for our objective. On the contrary, while the ERBB2 model did not deliver a significant p-value, it at least showed a weak positive correlation.

#Discussion
All in all, the multilinear regression models failed to meet the regression assumptions and did not deliver a p-value of significant relevance for the correlation. Thus, they should not be used for further predictions.
...
It seems possible that the small sample size for the regression model may lead to the lack of significant results, with only 28 events (CERES score observed in cell line) per predicting variable (SST gene).A common rule of thumb for building a regression model is to calculate the _limiting sample size_:
...
If this equation is fulfilled, the model is supposed to be reliable.(Harrell 2001). For our four models the condition was indeed shown to be true and thus our data seems to have an appropriate size for a basic regression model. However when the  supervised machine learning process of the regression is considered, sample sizes up to n = 50,000 are usualy reported.(Libbrecht and Noble 2015) It can thus be said that in general more data will lead to improved data quality, with a focus on obtaining and analyzing larger data sets instead of comparing different learning techniques on small training sets.(Banko and Brill 2001)
...
Based on the result of the Wilcoxon test, the top 15 genes with highest p-values for each mutation were chosen.  Due to some genes having the same p-value, ERBB2 and PARP10 were each selected to have 17 potential SST candidates, PIK3CA 18 potential candidates while MYCBP had 15 candidates.In the next step all potential SST candidates were verified, by checking their cluster assignment in the previously completed k means clustering. As expected, all SST candidates were confirmed to be found in in the same cluster 3 as the driver mutations. With this verification, all SSTs were characterized and labelled with mutation type and the physiological gene function using the universal protein database (UniProt). Most strikingly, many SST gene functions comprised proteins contributing to cell cycle progression, cell motility and polarity together with transcriptional regulators affecting cell metabolism. The most common type of mutation was a missense or silent mutation for SSTs of all driver mutations. *does this part need to be in more detail?* ( i dont think so . i will add some things about the database research, than it is enough)
