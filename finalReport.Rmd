...
author: "ELias Farr, Lennart Linke, Lisa Marie Milchsack, Salome Steinke"
date: "17. Juli 2019"
output: 
  html_document:
    highlight: pygments
...
If you grow up in an western country in the beginning 21th century, there are two sorts of tumor, you will definately notice before you leave high school. The first one is lung cancer, gaining its questionable fame with pictures leaping out of every box of cigarettes. The second one went famous with broadly reported stories about Angelina Jolie or famous old german universities. However, breast cancer isn't just the story of attractive women getting their breasts amputated or research institutes making themselves ridicolous, it is also a story of unthinkiable suffering and death.
We think about breast cancer as the most prevalent tumor disease worldwide(IHME GBD). It is also one of the most deathly malignant tumors in general with far more than 600.000 deaths, reported in 2017 (same origin). 
But there is hope. In the past decades, breast cancer therapy evolved, diminishing the death rate to ... percent and ... usw. 
Up to a point this is satisfying, on the other hand there are still a lot of untaken possibilities and new therapies to be tested. On group of methods increasing its powers every days are computerbased methods. until now, they cover about ... percent of this research fields with a clearly uprising tendency. This makes us all excited about what comes in the future. Until this future is reality, we will focus on our project, teaching ourselves in a nice manner, how useful a fast thinking computer might be in research. 
...
```{r pressure, echo=FALSE, fig.cap="Concept of SSTs", out.width = '100%', fig.align = "center"}
...
root.dir = "C:/Users/Salome/Documents/4.FS/Bioinfo/"
```
...
We made a heatmap of expression values *where is this heatmap?* and found a very diverse an unordered expression of genes trough the 28 breast cancer cell lines. Somehow, the CERES values showed a wide variety as well, giving us a clue how useful our computer based approach might be. To make shure we can use all cell lines, we investigated if some of the cell lines shows abnormal values in comparison to others. A few boxplots prooved this is not the case. 
Another issue was the very low level of expression of most genes. We discussed the possibility of erasing an amount of these genes and decided to not cut out any data, before we don't know  which kind of data we will need in the end. In this phase of the project is was open if a second site target will have a high expression in every case. 
...
To run Multilinear regression, we reform the BCCL CERES matrix. By transponing and format change, we obtain a matrix with colnames = Gene names and rownames = Cell line indices (equal to tPatients_ID). Then, we collect the relevant the CERES scores for the mutations for the linear regression.
...
The rho value of the MYCBP-model leads to very weak negative correlation. A p-value of ~ 0.78 leads to condifent acceptance of H0.
...
The rho value of the PIK3CA-model leads to very weak negative correlation. A p-value of ~ 0.78  leads to condifent acceptance of H0.
...
Especially the models for ERBB2 and PIK3CA should have been discarted, since not even the basic sample distribution is normallyn distributed.
...
Nevertheless ERBB2, MYCBP, PARP10 and PIK3CA were used as central driver mutations for further analysis. Box-and whisker plots of CERES scores revealed mutations of the MYCB gene to have greatest essentiality, with least variation, for cell survival. ERBB2 on the other hand had the highest average of CERES scores, greatest variation and lowest impact on cell proliferation. In contrast the ERBB2 driver mutation showed highest TPM values and hence suggested overexpression, however remaining the gene with greatest variation in values. TPM expression values were lowest for PIK3CA, with little variation together with MYCBP. 

Our approach had defined SSTs to be genes expected to interact synergistically with the corresponding driver mutation in promoting cell viability. Therefore, CERES values were expected to be similar to those of the corresponding driver mutation. As a consequence, all 1171 genes colocalized in k means cluster 3 were regarded as potential SST candidates and used as input for the statistical Wilcoxon test. 
