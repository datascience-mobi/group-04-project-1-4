...
processingfunction <- lapply(seq_along(SST_all_can), function(a) { # pick one dataframe fro list
  mutpicker <- SST_all_can[a] # define dataframe as mutpicker
    ...
    SST_all_can["SST Cluster Check"] <- sapply(1:nrow(mutpicker), function(b) {
    if (rownames(mutpicker) %in% rownames(data_clus_3e)) {
      SST_all_can[a,1] <- paste0("SST", " ", mutPicker)
    } else {
      SST_all_can[a,1] <- "No SST"  
    }
  })
  return(SST_all_can)
})
