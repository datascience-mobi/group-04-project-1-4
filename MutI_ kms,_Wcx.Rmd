...
#Paired Wilcoxon signed rank test -> not a good idea... doesn´t work as there are too many NAs
...
rownames2col <- function(inDF, RowName = ".rownames") {
  ...
  temp
} #this f(x) is used to add each subelement of each list (CGI_RDS or Non_CGI_RDS) and adds each case as a new column (so a case like: H2K72ac) to the matrix; if a value is for instance in a case m
...
head(finalFrame)
```
#show which genes are not potential SSTs
```{r}
...
#select eight most promising SST candidates based on lowest p-values - libraries needed: dplyr, tibble (included in "tidyverse") -> I will try and find a simpler version for this code.. I am pretty sure you can handle this with an apply command
SST_cand_ERBB2 <- finalFrame %>% select("ERBB2") %>% rownames_to_column() %>% filter(ERBB2 < quantile(ERBB2, probs = 0.001)) %>% column_to_rownames()
SST_cand_MYCBP <- finalFrame %>% select("MYCBP") %>% rownames_to_column() %>% filter(MYCBP < quantile(MYCBP, probs = 0.001)) %>% column_to_rownames()
SST_cand_PARP10 <- finalFrame %>% select("PARP10") %>% rownames_to_column() %>% filter(!is.na(PARP10)) %>% filter(PARP10 < quantile(PARP10, probs = 0.001)) %>% column_to_rownames() #removal of NA required 
SST_cand_PIK3CA <- finalFrame %>% select("PIK3CA") %>% rownames_to_column() %>% filter(PIK3CA < quantile(PIK3CA, probs = 0.001)) %>% column_to_rownames()
```